# WHAT IS FLASHWIZ APP?  
FlashWizApp is an android application that helps you learn English effectively using flashcards. With FlashWizApp, you can create your own flashcards or choose from a variety of pre-made decks. You can also track your progress and set reminders to help you stay on track.  

 # FEATURES  
- Create and customize flashcards  
- Access pre-made decks for various topics  
- Monitor your learning progress  
- Receive timely reminders to keep you motivated  
- Optimize learning with spaced repetition  
- Connect with friends and form study groups  
- Learn English effectively and efficiently  
  
# UI  
<img width="141" alt="login" src="https://github.com/user-attachments/assets/ef146a57-3c67-40e8-905e-e6d87f002def">  
<img width="142" alt="home" src="https://github.com/user-attachments/assets/478a806e-6580-4c42-b1c3-953cdbebcc15">  
<img width="142" alt="flashcard" src="https://github.com/user-attachments/assets/b01787dd-7333-4bb4-8814-700b4c2f8ac8">  
<img width="143" alt="data" src="https://github.com/user-attachments/assets/6b66d7d3-cc1d-4d0a-bc63-98ee54472735">  
<img width="142" alt="group" src="https://github.com/user-attachments/assets/8e158038-0bdb-44e3-8fc1-b77eeea0f202">  

  
# CLONE PROJECT  
 https://github.com/vodinhluan/flashwizapp.git  
# SET UP PROJECT  
- BACK-END SIDE: Change username and password (MySQL) in application.properties  
- FRONT-END SIDE: Change BASE_URL based on ID IPV4  


